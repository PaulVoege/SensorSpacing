...
class HeterPyramidCollab(nn.Module):
    ...
    def model_train_init(self):
        if self.args.get("stage2_online", False):
            self.eval()
            for p in self.parameters():
                p.requires_grad_(False)

            ego_modality = self.args['ego_modality']
            fix_ego_encoder = self.args.get('fix_ego_encoder', False)

            if fix_ego_encoder:
                trainable_modules = [f"self.aligner_{ego_modality}"]
            else:
                trainable_modules = [f"self.encoder_{ego_modality}", 
                                    f"self.backbone_{ego_modality}",
                                    f"self.aligner_{ego_modality}"]

            for trainable_module in trainable_modules:
                for p in eval(trainable_module).parameters():
                    p.requires_grad_(True)
                eval(trainable_module).apply(unfix_bn)
    ...
    def forward(self, data_dict):
        ...
        if self.rafe is not None:
            self.rafe(data_dict, heter_feature_2d)  
