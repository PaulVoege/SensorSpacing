...
from opencood.models.fuse_modules.sdvit.sd_fusion import SDFusionBlock
...
class SDFusion(nn.Module):
    def __init__(self, config):
        super(SDFusion, self).__init__()
        ...
        self.final_fuse = config['final_fuse'] # ego or maxpool

        input_dim = config['hetero_fusion_block']['input_dim']

        self.num_iters = config['num_iters']
        for i in range(self.num_iters):
            setattr(self, f"hetero_fusion_block_{i}",SDFusionBlock(
    ...
    def forward(self, x, pairwise_t_matrix, mode, record_len, mask):
        ...
        for i in range(self.num_iters):
            x = eval(f"self.hetero_fusion_block_{i}")(x, pairwise_t_matrix, temp,
                                         record_len, mask)
        # x = x[:, 0, ...]
        # (B, M, C, H, W) -> (B, C, H, W)
        if self.final_fuse == 'ego':
            x = x[:, 0, ...].permute(0, 2, 3, 1)
        elif self.final_fuse == 'max':
            x = torch.max(x, dim=1)[0].permute(0, 2, 3, 1)
        else:
...
class SDViT(nn.Module):
    def __init__(self, args):
        ...
        """
        self.fusion_net = SDFusion(args['sdvit'])
