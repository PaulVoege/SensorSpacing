...
class HeteroPreNorm(nn.Module):
    def __init__(self, dim, fn, num_types=2):
        super(HeteroPreNorm, self).__init__()
        self.norm = HeteroLayerNorm(dim, num_types=num_types)
        self.fn = fn

    def forward(self, x, *args, **kwargs):
        return self.fn(self.norm(x, *args), *args, **kwargs)
class HeteroPreNormResidual(nn.Module):
    def __init__(self, dim, fn, num_types=2):
        super(HeteroPreNormResidual, self).__init__()
        self.norm = HeteroLayerNorm(dim, num_types=num_types)
        self.fn = fn

    def forward(self, x, *args, **kwargs):
        return self.fn(self.norm(x, *args), *args, **kwargs) + x

class BaseSimpleHetero(nn.Module):
    def __init__(self, num_types=2):
        super().__init__()
...
class HeteroLayerNorm(BaseSimpleHetero):
    def __init__(self, dim, num_types=2):
        super(HeteroLayerNorm, self).__init__(num_types)
        self.num_types = num_types
        for i in range(num_types):
            self.add_net(nn.LayerNorm(dim))
    def forward(self, x, mode, *args, **kwargs):
        out = []
        temp = torch.clone(mode)
        for i in range(self.num_types):
            x_mode = x[temp == i, ...]
            x_mode = self.net[i](x_mode)
            out.append(x_mode)
        out = self.combine_features(out, temp)
        return out


class HeteroFeedForward(BaseSimpleHetero):
    def __init__(self, dim, hidden_dim, dropout=0., num_types=2, out_dim=None):
        super(HeteroFeedForward, self).__init__(num_types)
...
class CavAttention(nn.Module):
    ...
    def forward(self, x, mask, prior_encoding=None):
        # x: (B, L, C, H, W) -> (B, H, W, L, C)
        x = x.permute(0, 3, 4, 1, 2)
        # (B, H, W, 1, L) -> (B, 1, H, W, 1, L)
        mask = mask.unsqueeze(1)
...
class HGTCavAttention(nn.Module):
    ...
    def forward(self, x, mode, mask):
        ...
        # mask: (B, 1, H, W, 1, L)
        mask = mask.unsqueeze(1)
        mode = mode.to(torch.int)

        qkv = self.to_qkv(x, mode)
        # (B,M,L,L,C_head,C_head)
