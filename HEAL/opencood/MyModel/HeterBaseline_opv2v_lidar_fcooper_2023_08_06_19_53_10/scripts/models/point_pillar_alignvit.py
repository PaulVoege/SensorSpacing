...
# Test AlignViTBlock and AlignViTFusionBlock
...
from opencood.models.sub_modules.align_vit_block import AlignViTBlock, AlignViTFusionBlock
from opencood.models.fuse_modules.fusion_in_one import AttFusion
from opencood.utils.transformation_utils import normalize_pairwise_tfm

class PointPillarAlignViT(nn.Module):
    """
    F-Cooper implementation with point pillar backbone.
    """
    def __init__(self, args):
        super(PointPillarAlignViT, self).__init__()
        ...
        self.align_vit_block = AlignViTBlock(args['align_vit'])
        
        self.align_vit_fusion = False
        if 'align_vit_fusion' in args:
            self.align_vit_fusion = True
            self.align_vit_fusion_block = AlignViTFusionBlock(args['align_vit_fusion'])

        self.fusion_net = AttFusion(args['att']['dim'])
    ...
    def forward(self, data_dict):
        ...
        spatial_features_2d = self.align_vit_block(spatial_features_2d)
        if self.align_vit_fusion:
            spatial_features_2d = self.align_vit_fusion_block(spatial_features_2d, record_len, affine_matrix)
