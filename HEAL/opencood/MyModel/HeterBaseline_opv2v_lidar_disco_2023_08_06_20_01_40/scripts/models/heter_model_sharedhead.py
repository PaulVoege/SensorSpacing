...
class HeterModelSharedhead(nn.Module):
    ...
    def model_train_init(self):
        ...
        if not self.args[eval("self.stage2_added_modality")].get("fix_encoder", True):
            for p in eval(f"self.encoder_{self.stage2_added_modality}").parameters():
                p.requires_grad_(True)
            for p in eval(f"self.backbone_{self.stage2_added_modality}").parameters():
                p.requires_grad_(True)
            eval(f"self.encoder_{self.stage2_added_modality}").apply(unfix_bn)
            eval(f"self.backbone_{self.stage2_added_modality}").apply(unfix_bn)
